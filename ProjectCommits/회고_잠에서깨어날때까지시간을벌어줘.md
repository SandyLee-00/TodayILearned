...
개인적인 목표 : 메모리 관련 문제 잘 처리하기 지난 프로젝트에 오브젝트가 많아져서 마지막 날에 터졌다. 어느 부분에서 문제인지 파악하지 못하고 꺼지면 계속 키면서 시연 상황에서 만족스럽지 않았다.

다른 사람들이 쓴 코드 이해하고 쓰기 : 엔진, 그래픽스, 툴 부분 다른 친구들이 만들었다. 지난 프로젝트에 쓰는 것은 할 수 있었는데 어떻게 만들어져있는지 이해 못했다. 그래서 그 부분이 이상한 것에 대해 리포팅하거나 수정을 하지 못했다. 
...
2. C++문법을 학습하고 나니까 작성하는 코드를 이해하고 만들 수 있었다. 전 프로젝트는 그냥 기능이 되도록 하고 연결만 했다. 그리고 하다가 이거 이상한데 하면서 코드가 만들어져 있는 것에 끌려갔다. 하지만 이번에는 더 잘 만들 방법에 대해 고민한 것이 잘한 점이다. 
...
2. 가장 다른 것들과 연결이 많이 되어있는 플레이어 관련 부분 구현을 했다.
...
Object : Scene에서 Object를 돌리고 Object에서 Component들을 돌렸다. 

1. 각 Component가 템플릿으로 GetComponent 해서 써야해서 디버깅이 힘들었다. 
2. Scene에서 객체를 만들고 컴포넌트는 각 클래스 안에서 세팅했다. 각 객체의 정보를 다르게 설정하는 것이 불가능했다. 

Button :  Button이 OnButtonClicked()를 위해 계속 Update()에서 MouseEvent를 체크한다 
